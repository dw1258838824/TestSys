...
public class DictUtils
{
     ...
     * 获取字典缓存 根据值返回标签名称
     * 
     * @param key 参数键    dictValue 对应值
     * @return dictDatas 字典数据列表
     */
    public static String getDictCache(String key,String dictValue)
    {
        Object cacheObj = CacheUtils.get(getCacheName(), getCacheKey(key));
        if (StringUtils.isNotNull(cacheObj))
        {
            ...
            for(int i=0 ;i<DictDatas.size() ; i++) {
            	if(DictDatas.get(i).getDictValue().equals(dictValue)) {
            		return DictDatas.get(i).getDictLabel();
            	}
            }
        }
        return "";
