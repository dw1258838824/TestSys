...
public class UserController extends BaseController
{
    ...
    {
        startPage();
        user.getParams().put("noStudent", "1");
        List<User> list = userService.selectUserList(user);
        if(list.size()>0) {
	        for(User u : list) {
	        	List<Role> rlist = roleService.selectRolesByUserId(u.getUserId());
	        	String roles = "";
	        	for(Role r: rlist) {
	        		if(r.isFlag())
	        			roles += "/" + r.getRoleName();
	        	...
	        	u.getParams().put("roleNames", roles);
	        }
        }
    ...
    @ResponseBody
    public AjaxResult importData(MultipartFile file, boolean updateSupport,Integer isStudent) throws Exception
    {
        ExcelUtil<User> util = new ExcelUtil<User>(User.class);
        List<User> userList = util.importExcel(file.getInputStream());

        if(null != isStudent && isStudent == 1) {//如果是导入学生
	        Long [] roleIds = {100L};
	        for(User u: userList) {
	        	u.setRoleIds(roleIds);
	        	u.setPhonenumber(u.getLoginName());
	        }
        }
        
        String message = userService.importUser(userList, updateSupport , isStudent);
    ...
    public AjaxResult editSave(@Validated User user)
    {
        ...
        return toAjax(userService.updateUserAll(user));
    }
